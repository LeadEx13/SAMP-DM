...
{
	// Time
	{"morning",1},{"noon",1},{"evening",1},{"midnight",1},{"settime",1},{"satime",1},
	// Kick/Ban/Unban
	{"kick",2},{"kickall",8},
	{"ban",2},{"tempban",3},
	{"unban",2},{"unbanip",3},{"banned",2},{"bannedip",2},{"untb",3},{"untbip",3},
	// Goto/Get
	{"goto",2},{"get",2},{"ptp",4},
	// Mute/Jail/Freeze/Explode/Slap/Kill
	{"mute",2},{"unmute",2},{"muted",2},
	{"jail",2},{"unjail",2},{"jailed",2},
	{"freeze",2},{"unfreeze",2},{"fall",6},{"unfall",6},
	{"slap",2},{"akill",3},{"explode",5},
	// HealthArmor/Vehicle
    {"full",2},{"sethp",2},{"setar",2},{"healall",4},{"armorall",4},{"god",4},
	{"flip",1},{"carhealth",2},{"vr",2},{"vrall",5},{"carcolor",1},{"eject",2},{"ejectall",3},{"givecar",6},
	{"gn",5},{"xlock",4},{"xunlock",4},{"xunlockall",5},
	// Money
	{"setmoney",8},{"givemoney",8},{"remmoney",8},{"resetmoney",8},{"samoney",9},{"gamoney",9},{"remamoney",9},{"ramoney",9},
	// Weapon/Ammo
	{"giveweapon",2},{"gaweapon",3},{"raweapons",5},{"disarm",4},{"getslot",2},{"setammo",2},
	// Skin
	{"setskin",1},{"skinall",2},
	// Name/Account/PlayerInfo/Server
	{"setname",10},{"saccname",10},{"changepass",1},
	{"ip",3},{"ping",3},
	{"uconfig",10},{"setsm",10},{"gmx",10},{"sgravity",10},{"setping",10},{"cfgvar",10},{"xvar",10},
	// World/Scores/Wanted
	{"setworld",5},{"saworld",10},{"outsideall",8},
	{"setscore",8},{"rscores",8},
	{"setwanted",5},{"sawanted",7},
	// Announcements/ChatControl
	{"ann",1},{"say",1},{"saa",2},{"snatime",10},{"snatext",8},{"sna",5},
	{"c",1},{"ca",2},{"chat",3},{"ac",9},{"lpt",6},
	// Commands/PMs
	{"pcmds",9},{"acmds",9},{"ppms",2},
	// Admin/CD/Spec/AdminVehicle/Other
	{"setlevel",10},{"tmplevel",10},{"rc",1},{"aafk",1},{"inv",1},
	{"cd",1},{"kcd",1},
	{"xspec",1},
	{"spawnc",3},{"spawnd",3},{"spawncall",6},
	{"plusz",1},{"force",5},
	// Teleport
	{"ct",4},{"ctd",4},{"ctl",2}
};
...
// Main includes
#include "xadmin\Core\XtremeAdmin.inc"
#include "xadmin\Core\XAP.inc"
...
public OnFilterScriptInit()
{
	...
	File = "/xadmin/Configuration/Announcements.ini";
	if(!dini_Exists(File))
	{
		dini_Create(File);
	...
	File = "/xadmin/Configuration/Configuration.ini";
	if(!dini_Exists(File))
	{
		dini_Create(File);
	...
	// Create Level Config:
	/*File = "/xadmin/Configuration/Commands.ini";
		...
		if(IsPlayerConnected(i))
		{
			...
			if (Variables[i][Level])
			{
				if (!Variables[i][LoggedIn])
					...
					SendClientMessage(i, orange, "The administration filterscript has been restarted.");
			}
		}
	}
	xVars[Reload] = false;
	...
	if (!xVars[LoadFailed])
	{
		...
		for(new i = 0; i < MAX_PLAYERS; ++i)
		{
			if(IsPlayerConnected(i))
			{
				...
				if (!xVars[Reload]) if (xPlayer[i][AdminVehicle]) dini_IntSet("/xadmin/DataSave/AVehicle.ini",name,xPlayer[i][AdminVehicle]);
				if (xPlayer[i][TMPLevel]) dini_IntSet("/xadmin/DataSave/TMPLevel.ini",name,Variables[i][Level]);
				if (IsPlayerXAdmin(i))
				{
					new fileline[128];
		...
		if(dini_Exists("/xadmin/CT.ini")) dini_Remove("/xadmin/CT.ini"); dini_Create("/xadmin/CT.ini");
		OpenChats();
...
public OnPlayerConnect(playerid)
{
	...
	{
		if (strcmp(dini_Get("/xadmin/Configuration/Configuration.ini","ServerMessage"),"None",true))
		{
			format(string,128,"Server Message: %s",dini_Get("/xadmin/Configuration/Configuration.ini","ServerMessage"));
	...
	if (!IsBanned)
	{
		...
		if(Config[DisplayConnectMessages] && !Variables[playerid][Level] && !xVars[Reload])
		{
			format(string,128,"*** %s has joined the server.",PlayerName);
			for(new i = 0; i < MAX_PLAYERS; ++i)
	...
	format(BanFile, 128, "/xadmin/Bans/Names/%s.ini", PlayerName);
	format(IpBanFile, 128, "/xadmin/Bans/IP/%s.ini", UserIP);
	if (dini_Exists(BanFile))
	{
		if (!dini_Exists(IpBanFile))
		{
			new old_IpBanFile[64]; format(old_IpBanFile,64,"/xadmin/Bans/IP/%s.ini",dini_Get(BanFile,"IP")); dini_Remove(old_IpBanFile);
	...
	else if (dini_Exists(IpBanFile))
	{
		if (dini_Exists(GetPlayerFile(playerid)) && dini_Int(GetPlayerFile(playerid), "Level"))
		{
			...
			SetKick(playerid, 2); return SendMessageToAdmins(yellow,string);
		}
		SendClientMessage(playerid,red,"You are banned from this server.");
...
//	new bool:IsBanned = (BanCheck(PlayerName,1)||BanCheck(UserIP,2)||TempBanCheck(PlayerName,1)||TempBanCheck(UserIP,2))?true:false;
	...
	if (Config[DisplayConnectMessages] && !Variables[playerid][Level] && !xVars[Reload]/* && !IsBanned*/)
	{
		format(string,128,"*** %s has left the server. (%s)",PlayerName,Reason);
	...
	if (reason != 2)
	{
		DSFile = "/xadmin/DataSave/Jail.ini";
		...
		DSFile = "/xadmin/DataSave/Mute.ini";
...
public OnPlayerSpawn(playerid)
{
	...
	{
		SendClientMessage(playerid,red,"You must be logged in before you can spawn!"); SetKick(playerid, 2);
	}
...
public OnPlayerCommandText(playerid,cmdtext[]) {
	...
	if((cmdtext[0] == '/' && cmdtext[1] == '/') && IsPlayerXAdmin(playerid)) {
		if (!xVars[AdminChat] && Variables[playerid][Level] < ReturnCommandLevel("ac")) return SendClientMessage(playerid,red,"* The admin chat is closed now.");
	...
	// Create Teleport
	new cmdtemp[128],tidx,CTFile[64] = "/xadmin/CT.ini"; cmdtemp = strtok(cmdtext,tidx);
...
dcmd_sm(playerid,params[])
{
	...
	if(Config[DisplayServerMessage]) {
		new string[128], File[64] = "/xadmin/Configuration/Configuration.ini";
...
dcmd_setsm(playerid,params[])
{
	if(IsPlayerCommandLevel(playerid,"setsm")) {
		...
		new File[64] = "/xadmin/Configuration/Configuration.ini";
...
dcmd_setmoney(playerid,params[])
{
	if(IsPlayerCommandLevel(playerid,"setmoney")) {
		...
		if(IsPlayerConnected(id) && id != INVALID_PLAYER_ID) {
			...
			FAC_ResetPlayerMoney(id);
			FAC_GivePlayerMoney(id,strval(tmp2));
			return 1;
...
dcmd_resetmoney(playerid,params[])
{
	if(IsPlayerCommandLevel(playerid,"resetmoney")) {
		...
		if(IsPlayerConnected(id) && id != INVALID_PLAYER_ID) {
			...
			return FAC_ResetPlayerMoney(id);
...
dcmd_samoney(playerid,params[])
{
	if(IsPlayerCommandLevel(playerid,"samoney")) {
		...
		for(new i = 0; i < MAX_PLAYERS; ++i) if(IsPlayerConnected(i)) {
			FAC_ResetPlayerMoney(i);
			FAC_GivePlayerMoney(i,strval(params));
		}
		new string[128],name[24]; GetPlayerName(playerid,name,24);
...
dcmd_ramoney(playerid,params[])
{
	if(IsPlayerCommandLevel(playerid,"ramoney")) {
		...
		for(new i = 0; i < MAX_PLAYERS; ++i) if(IsPlayerConnected(i)) FAC_ResetPlayerMoney(i);
		new string[128],name[24]; GetPlayerName(playerid,name,24);
...
dcmd_setlevel(playerid,params[])
{
	if(IsPlayerCommandLevel(playerid,"setlevel")) {
		...
		if(!strlen(tmp)||!strlen(tmp2)||!IsNumeric(tmp2)||!(strval(tmp2) >= 0 && strval(tmp2) <= Config[MaxLevel])) {
			new string[128];
			format(string,128,"Syntax Error: \"/SETLEVEL <NICK OR ID> <0 - %d>\".",Config[MaxLevel]);
		...
		if(IsPlayerConnected(id) && id != INVALID_PLAYER_ID && id != playerid) {
			...
			new string[128],name[24],ActionName[24]; GetPlayerName(playerid,name,24); GetPlayerName(id,ActionName,24);
			SendACommandMessageToAdmins(playerid,"setlevel",params);
		...
		if(IsPlayerConnected(id) && id != INVALID_PLAYER_ID && id != playerid) {
			if(!Variables[id][Muted]) {
				...
				if(strval(tmp2) > 0) {
					...
					if (!strlen(reason)) format(string,128,"\"%s\" has been muted by Administrator \"%s\" for %s.",ActionName,name,nTime);
					else
					{
						format(string,128,"\"%s\" has been muted by Administrator \"%s\" for %s. (Reason: %s)",ActionName,name,nTime,reason);
						format(xPlayer[id][MuteInfo][mReason], 32, reason);
					}
				} else {
					if (!strlen(reason)) format(string,128,"\"%s\" has been muted by Administrator \"%s\".",ActionName,name);
					else
					{
						format(string,128,"\"%s\" has been muted by Administrator \"%s\". (Reason: %s)",ActionName,name,reason);
						format(xPlayer[id][MuteInfo][mReason], 32, reason);
					}
				}
	...
	if(IsPlayerCommandLevel(playerid,"unmute")) {
		...
		if(IsPlayerConnected(id) && id != INVALID_PLAYER_ID) {
			if(Variables[id][Muted]) {
				...
				#endif
				if(id != playerid) {
					format(string,128,"\"%s\" has been unmuted by Administrator \"%s\".",ActionName,name); return SendClientMessageToAll(yellow,string);
...
dcmd_c(playerid, params[])
{
	if(IsPlayerCommandLevel(playerid,"c")) {
		...
		SendMessageToPlayers(green, ".שקט בבקשה");
...
dcmd_cd(playerid,params[])
{
	if(IsPlayerCommandLevel(playerid,"cd")) {
		...
		format(string, 128, "Countdown started; Administrator: \"%s\", Time: %s", aName, nTime);
...
dcmd_snatime(playerid,params[])
{
	if(IsPlayerCommandLevel(playerid,"snatime")) {
		...
		if (strval(params) == 0) {
			if(!dini_Bool("/xadmin/Configuration/Announcements.ini","System")) return SendClientMessage(playerid,red,"ERROR: The system is already off.");
			KillTimer(xVars[TimerNews]); xVars[TimerNews] = -1;
			dini_BoolSet("/xadmin/Configuration/Announcements.ini","System",false);
			...
			dini_BoolSet("/xadmin/Configuration/Announcements.ini","System",true);
			dini_IntSet("/xadmin/Configuration/Announcements.ini","Time",strval(params));
...
dcmd_snatext(playerid,params[])
{
	if(IsPlayerCommandLevel(playerid,"snatext")) {
		new File[64]; File = "/xadmin/Configuration/Announcements.ini";
...
dcmd_sna(playerid,params[])
{
	if(IsPlayerCommandLevel(playerid,"sna")) {
		new File[64]; format(File,64,"/xadmin/Configuration/Announcements.ini");
		if(!dini_Bool(File,"System")) return SendClientMessage(playerid,red,"ERROR: The news system is off.");
...
dcmd_unban(playerid,params[])
{
	if(IsPlayerCommandLevel(playerid,"unban")) {
		...
		new BanFile[64], IpBanFile[64]; format(BanFile, 64, "/xadmin/Bans/Names/%s.ini", params);
		new breason[128]; format(breason,128,dini_Get(BanFile, "Reason"));
		new string[128], File[64]; format(string, 128, "You have unbanned \"%s\".", params); format(File, 64, "/xadmin/Bans/Names/%s.ini", params);
		...
		if (dini_Exists(BanFile)) { format(IpBanFile, 64, "/xadmin/Bans/IP/%s.ini", dini_Get(BanFile, "IP")); if (dini_Exists(IpBanFile)) {
...
dcmd_unbanip(playerid,params[])
{
	if(IsPlayerCommandLevel(playerid,"unbanip")) {
		...
		new IpBanFile[64], BanFile[64]; format(IpBanFile, 64, "/xadmin/Bans/IP/%s.ini", params);
		new string[128], File[128];
		format(string, 128, "You have unbanned %s (\"%s\").", dini_Get(IpBanFile, "PlayerName"), params); format(File,64,"/xadmin/Bans/IP/%s.ini",params);
		new fileline[128],name[24]; GetPlayerName(playerid,name,24);
		...
		if (dini_Exists(IpBanFile)) { format(BanFile, 64, "/xadmin/Bans/Names/%s.ini", dini_Get(IpBanFile, "PlayerName")); if (dini_Exists(BanFile)) {
...
{
	if(IsPlayerCommandLevel(playerid,"changepass")) {
		if (Variables[playerid][LoggedIn] && Variables[playerid][Level]) {
			new astriks[16]; for (new i; i<strlen(params); ++i) astriks[i] = '*';
			SendACommandMessageToAdmins(playerid,"changepass",astriks);
			new string[128];
			if (!strlen(params)) return SendClientMessage(playerid,red,"Syntax Error: \"/CHANGEPASS <NEW PASSWORD>\".");
	...
	} else return SendLevelErrorMessage(playerid,"changepass");
	return 1;
}
...
dcmd_banned(playerid,params[])
{
	if(IsPlayerCommandLevel(playerid,"banned")) {
		...
		if (!BanCheck(params, 1)) return SendClientMessage(playerid, red, "This player is not banned.");
		new string[128], File[64]; format(File, 128, "/xadmin/Bans/Names/%s.ini", params);
...
dcmd_bannedip(playerid,params[])
{
	if(IsPlayerCommandLevel(playerid,"bannedip")) {
		...
		if (!BanCheck(params, 2)) return SendClientMessage(playerid, red, "This IP is not banned.");
		new string[128], File[128]; format(File, 128, "/xadmin/Bans/IP/%s.ini", params);
...
public OnPlayerText(playerid, text[])
{
	...
	{
		if (!xVars[AdminChat] && Variables[playerid][Level] < ReturnCommandLevel("ac")) return SendClientMessage(playerid,red,"* The admin chat is closed now."), 0;
	...
	if(Variables[playerid][Muted] && strcmp(text,"$",false))
	{
		Variables[playerid][MutedWarnings]--;
		new string[128],Name[24];
		if(Variables[playerid][MutedWarnings]) {
			format(string,128,"You have been muted, this preventing you from talking or PMing. [Warnings: %d/%d]",Variables[playerid][MutedWarnings],Config[MutedWarnings]);
			SendClientMessage(playerid,white,string); return 0;
		} else {
			GetPlayerName(playerid,Name,24); format(string,128,"%s has been Kicked from the server. (Reason: Muted)",Name);
			SendClientMessageToAll(yellow,string); Variables[playerid][Muted] = false;
...
public OnRconCommand(cmd[])
{
	...
	if(strcmp(cmdtmp, "xbanned", true)==0)
	{
		...
		new File[128]; format(File, 128, "/xadmin/Bans/Names/%s.ini", tmp);
	...
	if(strcmp(cmdtmp, "xbannedip", true)==0)
	{
		...
		new File[128]; format(File, 128, "/xadmin/Bans/IP/%s.ini", tmp);
	...
	if(strcmp(cmdtmp, "xunban", true)==0)
	{
		...
		if (!BanCheck(tmp, 1)) return printf("The player \"%s\" is not banned.",tmp), 1;
		new BanFile[128], IpBanFile[128]; format(BanFile, 128, "/xadmin/Bans/Names/%s.ini", tmp);
		...
		if (dini_Exists(BanFile))
		{
			format(IpBanFile, 128, "/xadmin/Bans/IP/%s.ini", dini_Get(BanFile, "IP"));
			if (dini_Exists(IpBanFile))
			{
				dini_Remove(BanFile);
	...
	if(strcmp(cmdtmp, "xunbanip", true)==0)
	{
		...
		if (!BanCheck(tmp, 2)) return printf("The IP \"%s\" is not banned.",tmp), 1;
		new IpBanFile[128], BanFile[128]; format(IpBanFile, 128, "/xadmin/Bans/IP/%s.ini", tmp);
		new fileline[128]; format(fileline,128,"RCON unbanned %s (%s) [Reason: %s]\r\n",tmp,dini_Get(IpBanFile, "PlayerName"),dini_Get(BanFile, "Reason"));
		WriteToLog("unbans", fileline);
		printf("You have unbanned %s (\"%s\").", dini_Get(IpBanFile, "PlayerName"), tmp);
		if (dini_Exists(IpBanFile))
		{
			format(BanFile, 128, "/xadmin/Bans/Names/%s.ini", dini_Get(IpBanFile, "PlayerName"));
			if (dini_Exists(BanFile))
			{
				dini_Remove(IpBanFile);
				dini_Remove(BanFile);
...
public OpenChats()
{
	...
	for (new i = 0; i < MAX_PLAYERS; ++i)
	{
		...
		{
			if (Variables[i][Level] >= ReturnCommandLevel("chat"))
				CC++;
			if (Variables[i][Level] >= ReturnCommandLevel("ac"))
				ACC++;
		}
	}
...
{
	new File[64] = "/xadmin/Configuration/Announcements.ini";
...
// => Include XAP Systems:
#if APS
	#include "xadmin\Systems\AP_System.inc"
#endif
#include "xadmin\Systems\Kick-Ban_System.inc"
#include "xadmin\Systems\Mute-Jail_System.inc"
#include "xadmin\Systems\Log_System.inc"
#include "xadmin\Systems\Spectate_System.inc"
#include "xadmin\Systems\Ban_System.inc"
#include "xadmin\Systems\XCMDS_System.inc"
//================================/=/==E==N==D==/=/=============================
